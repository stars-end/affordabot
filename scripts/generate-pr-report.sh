...
# Generate the report
cat > "$REPORT_FILE" << EOF
## 🔬 Verification Report for PR #${PR_NUMBER}
...
if [ -f "$RESULTS_FILE" ] && [ "$(cat $RESULTS_FILE | jq -r '.phases | length')" != "0" ] 2>/dev/null; then
    cat "$RESULTS_FILE" | jq -r '.phases[] | "| \(.name) | \(if .passed then "✅ PASS" else "❌ FAIL" end) | \(.duration)s |"' >> "$REPORT_FILE"
else
    ...
    if [ -f "artifacts/verification/discovery/discovery_report.json" ]; then
        ...
        DB_STATUS=$(echo "$DISCOVERY" | jq -r 'if .results.db then "✅ PASS" else "❌ FAIL" end')
        PROMPT_STATUS=$(echo "$DISCOVERY" | jq -r 'if .results.prompt then "✅ PASS" else "❌ FAIL" end')
        QUERIES_STATUS=$(echo "$DISCOVERY" | jq -r 'if .results.queries then "✅ PASS" else "❌ FAIL" end')
        OVERALL=$(echo "$DISCOVERY" | jq -r 'if .passed then "✅ PASS" else "❌ FAIL" end')
    ...
    if [ -f "artifacts/verification/affordabot_san_jose_pipeline_report.md" ]; then
        echo "| San Jose Pipeline | ✅ Report Generated | - |" >> "$REPORT_FILE"
    fi
fi
...
<sub>Generated by `make verify-pr PR=${PR_NUMBER}`</sub>
EOF

# Replace placeholder in merge command
sed -i "s/\${PR_NUMBER}/${PR_NUMBER}/g" "$REPORT_FILE"
