...
- generated_at: `2026-04-17T02:52:27.681236+00:00`
...
## Manual Run
- attempted: `true`
- idempotency_key: `bd-3wefe.13-live-cycle-31-20260417025026`
- windmill_job_id: `019d9958-ef07-5e79-1667-969968a0e235`
...
- stale_drill_stale_but_usable: `passed` ({'idempotency_key': 'bd-3wefe.13-live-cycle-31-20260417025026:stale_but_usable', 'requested_stale_status': 'stale_but_usable', 'status': 'succeeded', 'scope_succeeded': 1, 'scope_failed': 0, 'step_sequence': ['search_materialize', 'freshness_gate', 'read_fetch', 'index', 'analyze', 'summarize_run'], 'freshness_gate_status': 'succeeded_with_alerts', 'freshness_gate_reason': 'stale_but_usable', 'read_fetch_status': 'succeeded', 'read_fetch_reason': 'raw_scrapes_materialized', 'quality_blocked': False})
- stale_drill_stale_blocked: `passed` ({'idempotency_key': 'bd-3wefe.13-live-cycle-31-20260417025026:stale_blocked', 'requested_stale_status': 'stale_blocked', 'status': 'failed', 'scope_succeeded': 0, 'scope_failed': 0, 'step_sequence': ['search_materialize', 'freshness_gate', 'summarize_run'], 'freshness_gate_status': 'blocked', 'freshness_gate_reason': 'stale_blocked', 'read_fetch_status': None, 'read_fetch_reason': None, 'quality_blocked': False})
...
| exa | succeeded | 5 | 1181 |  | https://www.sanjoseca.gov/Home/Components/News/News/1683/4765 |
| tavily | succeeded | 5 | 444 |  | https://www.sanjoseca.gov/your-government/departments-offices/housing/developers/commercial-linkage-fee |
...
- llm_analysis_excerpt: The provided evidence offers a high-level overview of the San Jose Commercial Linkage Fee (CLF) program, detailing its purpose as an affordable housing funding mechanism, its implementation timeline, and the method for annual adjustments via the ENR Construction Cost Index. However, it fails to deliver the specific source-bound economic parameters—such as actual fee rates per square foot or precise subarea definitions—necessary for rigorous cost-of-living analysis. Additionally, the text lacks structured source lineage, missing citations to the specific Fee Resolution or primary municipal codes.
